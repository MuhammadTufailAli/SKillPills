@extends('template')

@section('content')
<link rel="stylesheet" " href=" /css/exercises.css">
<section class="content-header">
    <h1>Exercise
        <small>&nbsp;</small>
    </h1>
    <span class="breadcrumb">
        <span class="fa fa-home margin1">
            <a href="" style="font-family: 'Sans Pro','Helvetica Neue',Helvetica,Arial,sans-serif; font-weight:400">
                &nbsp;Home
            </a>
        </span>
        <span>
            <span class="fas fa-caret-right">

            </span>
        </span>
        <span class="fa fa-book margin">
            <span style="font-family: 'Sans Pro','Helvetica Neue',Helvetica,Arial,sans-serif; font-weight:400">
                &nbsp;Exercise</span>

        </span>

    </span>
    <br>
    <br>

</section>

<section id="main" class="content">
    <div class="row no-margin">
        <div class="panel-group">
            <div class="col-md-12 col-lg-12">
                <h4 class="alert alert-danger">
                    Dear Trainee! You have no assignment in current Batch at the moment.
                </h4>
                <button style="float: right;background-color: #008CBA;font-size: 18px;"><a style="text-decoration: none;color:white;" href="/userinterface">Back</a></button>

            </div>

        </div>

    </div>
</section>

<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>
<h1 style="color: #ecf0f5;">SAMPLE TEXT</h1>


@endsection